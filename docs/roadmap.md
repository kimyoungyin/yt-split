...
## Phase 3 — 프로젝트 라이브러리 + AppLocalData (취소는 Phase 4)

**무엇을:** 한 번 처리한 곡들을 라이브러리로 누적하고, 다음 실행에서 다시
열 수 있게 한다. 출력 위치를 워크스페이스 루트 → 사용자별 AppLocalData로
옮긴다. 현재 범위 잠금은 `P3-A + P3-B` 우선 확정이며, `P3-C/P3-D`는 후속
사이클로 분리한다.
...
- 1차(잠금): `P3-A/P3-B` 중심 (`--workdir`, AppLocalData base, 프로젝트 메타/경로 규약).
- 2차(후속): `ui/src/entities/project/`, `ui/src/features/library/` (`P3-C`).
- Rust 측: `tauri::path::PathResolver::app_local_data_dir()`로 base 경로
  결정. dev에서도 동일 위치 사용해 dev/prod 일관성 확보.
  사이드카 spawn 시 `--workdir <path>` 인자로 base를 전달하고, Python은
  `Path.cwd()` 의존 대신 인자를 우선 사용.
- 메타데이터: `<base>/projects/<uuid>.json` (제목, URL, 생성 시각, device,
  트랙 경로). 디렉터리 스캔으로 라이브러리 리스트. `tracks` 값은 **프로젝트 루트**
  `<base>/projects/<uuid>/` 기준 상대 경로(예: `stems/vocals.wav`). NDJSON
  `stage.done(separate)`에는 UI 즉시 재생용 **절대 경로 `tracks`를 포함한다(잠금)**
  (메타와 채널이 다름 — [`docs/phase-3.md`](./phase-3.md) P3-B).
- 경로 보안: 메타에서 읽은 상대 경로에 `..` 금지. 복원한 절대 경로를
  canonicalize 한 뒤 `<base>/projects/<id>/` 하위인지 검증(Rust/Python 동일 규칙).
- 출력 전략: demucs 기본 경로는 `-o <base>/projects/<uuid>/` direct output으로
  단일화, post-move는 fallback 복구 경로에서만 사용.

**Risk:**

- `Path.cwd()` 기본값: CLI/사이드카 모두 P3-A에서 `--workdir`가 있으면 그걸
  `base_dir`·`check_path`에 쓰고, 없으면 기존처럼 cwd(회귀 방지). 사이드카 spawn
  시 Rust가 `app_local_data_dir()/yt-split`을 넘긴다.
- macOS Sandbox / Hardened Runtime: AppLocalData 쓰기는 OK. 이후 Phase 5에서
  notarization 시 entitlements 다시 점검.
- 메타 손상/부분 실패 시 orphan(stems만 있고 json 없음) 복구 정책 필요
  (atomic write + 재시도/복구 표시).
- 동시 실행/취소 레이스 방지: 단일 실행 정책(queue 또는 busy 반환) 명시 필요.
...
- **사이드카 취소(특히 Windows)는 Phase 4와 함께 처리 권장**: 프로세스 제어 전략
  (`jobobject`/`GenerateConsoleCtrlEvent`)과 패키징 환경 검증을 같은 사이클로 묶는 편이 시행착오가 적다.
