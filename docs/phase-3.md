...
- **프로젝트 디렉터리 레이아웃:**
    ...
    ```
    Demucs 기본 트리(`…/htdemucs/<title>/…`)는 사용자에게 노출하지 않는다. **기본 전략:** `demucs -o <base>/projects/<uuid>/`로 두고, 최종 stem 파일은 `<base>/projects/<uuid>/stems/*.wav`에 두거나( Demucs가 부모 아래에 `htdemucs/…`를 만들면) **정리 단계에서 `stems/`로 옮겨** 라이브러리 규약을 맞춘다. post-move는 § 인터페이스 노트의 fallback(예외 복구) 경로에서만 사용한다.
- **프로젝트 메타데이터 스키마 v1:**
    ```json
    {
      "id": "<uuid v4>",
    ...
    ```
    `tracks` 값은 **프로젝트 루트** `<base>/projects/<uuid>/` 기준 상대 경로(예: `stems/vocals.wav`). 메타 파일은 `projects/<uuid>.json`에 두되, 경로 합성은 항상 `project_dir = <base>/projects/<uuid>` 후 `project_dir / Path(rel)` 로 한다. `<base>`만 바뀌면 동일 상대 경로로 이전 가능하다.
...
| `src/features/project.py` (신규)                                                                      | `create_project_metadata(base, *, project_id, title, url, device, stem_mode, tracks: dict[str, Path]) -> Path`. 호출 시 `tracks`는 **절대 경로**; 함수는 디스크 메타에만 프로젝트 루트 기준 상대 경로로 기록. `list_projects(base) -> list[ProjectMeta]`. (`project_id`는 호출부에서 `uuid.uuid4()` 생성해 전달.) |
| [`src/app/main.py`](../src/app/main.py)                                                               | 분리 완료 후 위 함수로 메타 작성. NDJSON `stage.done(separate)`에는 **`project_id`**, **`title`**, **절대 경로 `tracks`**를 포함한다(잠금). 디스크의 `tracks` 상대 경로와 동일 정보를 두 번 저장하는 것이 아니라, **저장소는 메타 1곳**, NDJSON은 UI 동기 갱신용이라 채널이 다르다. |
...
- **상대 경로 vs 절대 경로:** 디스크 메타(`*.json`)에는 프로젝트 루트 기준 상대 경로만 둔다. NDJSON은 파이프라인 직후 UI가 Player를 즉시 갱신할 수 있도록 **절대 경로 `tracks`를 반드시 포함한다(잠금)**. 라이브러리·재시작 후에는 메타 + base resolver로 절대 경로를 복원한다.
- **경로 검증 규칙(필수):** 메타에서 읽은 각 상대 세그먼트에 `..`가 있으면 즉시 거부. 복원한 절대 경로를 canonicalize 한 뒤, 정규화된 `<base>/projects/<id>/prefix` 아래에 있어야 한다(플랫폼별 경로 비교 시 동일 규칙을 Rust/Python 양쪽에 둔다).
- **two-stem 모드:** Demucs `--two-stems vocals` 는 보통 `vocals.wav`와 `no_vocals.wav`(또는 Demucs 버전에 따른 동등 파일) 2개를 남긴다. 실제 파일명은 출력 디렉터리에서 `*.wav` 스캔으로 수집한다(현행 [`_resolve_output_tracks`](../src/features/separation.py) 패턴 유지). 메타의 `tracks`에는 존재하는 stem 키만, `stem_mode`는 CLI에 넘긴 stem 이름(예: `"vocals"`)으로 기록한다.
- **다운로드 캐시:** 같은 URL을 다시 처리하면 새 uuid + 새 mp3. 중복 검출은 Phase 3 범위 밖 (open decision OD-2 참조).
...
def test_create_project_metadata_writes_v1_schema(tmp_path):
    ...
    assert data["tracks"]["vocals"] == "stems/vocals.wav"  # 프로젝트 루트 기준 상대
```
...
| #        | 항목                                   | 옵션                                                                         | 현재 추천                                                                                                                              | 상태         |
| -------- | -------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| **OD-1** | 다운로드 mp3를 메타에 함께 보존?       | (a) 보존, 메타에 `source_mp3` 경로 (b) 분리 후 삭제                          | **(a)**. 추후 다른 모델로 재분리 가능. 디스크 비용은 Demucs 출력 대비 작음                                                             | **Locked**   |
| **OD-2** | 같은 URL 재처리 시 동작                | (a) 항상 새 uuid (b) 기존 프로젝트 덮어쓰기 (c) 사용자에게 묻기              | **(a)**. 단순. 중복 정리는 Phase 4+.                                                                                                   | **Locked**   |
| **OD-3** | 메타 v1 → v2 마이그레이션 정책         | (a) `schema_version` 읽고 변환 함수 (b) 무시하고 새 스키마로 강제            | Phase 3에서는 (a) hook만 만들고, 실제 v2까지는 nope.                                                                                   | **Locked**   |
| **OD-4** | `delete_project`가 raw mp3까지 지우나? | OD-1과 연동                                                                  | OD-1=(a)면 함께 삭제. 단, 구현은 2-phase delete 기준으로 실패 복구/재시도 상태를 노출한다.                                             | **Locked**   |
...
| **OD-7** | `<base>/yt-split/` 중첩이 적절한가?    | (a) `app_local_data_dir() / "yt-split"` (b) `app_local_data_dir()` 직접 사용 | **(a)** — § 확정된 결정과 동일. 서브폴더로 downloads/projects/logs를 묶어 백업·정리 단위를 통일한다. | **Locked**   |
...
- [x] **OD-1 잠금:** mp3 보존 정책 확정(`source_mp3` 포함 여부)
- [x] **OD-2 잠금:** 동일 URL 재처리 정책 확정(기본: 항상 새 uuid)
- [x] **OD-3 잠금:** schema_version hook 범위 확정(v1 읽기 + v2 확장 훅만)
- [x] **OD-4 잠금:** delete 시 raw mp3 삭제 연동 정책 확정(OD-1과 일치)
- [ ] **OD-7 잠금:** `<base>/yt-split/` 중첩 — Open decisions 표 **Locked**와 동일
...
- [ ] 단일 실행 정책 명시(queue 또는 busy) — Python·Rust·UI 중 한 곳에서라도 일관되게 시행
- [ ] cancel 대상 PID 교차 방지 — P3-B에서 sidecar 단일 슬롯이면 P3-A 이후부터 적용 가능
- [ ] delete 2-phase(마킹→삭제→실패 시 복구/재시도) — **UI·명령은 P3-C**, **파일 규약·테스트는 P3-B에서 정의** 가능
- [ ] orphan(stems만 있고 json 없음) 탐지 — 라이브러리 스캔(P3-C) 전에 P3-B 메타 실패 경로와 맞출 것
...
- **Tauri `$APPLOCALDATA`와 workdir 일치:** Tauri v2에서 `$APPLOCALDATA`는 앱 로컬 데이터 디렉터리(보통 `app_local_data_dir()`와 동일 계열)로 풀린다. workdir를 `app_local_data_dir()/yt-split`로 두었다면 scope는 `["$APPLOCALDATA/yt-split/**"]`가 맞는지 **실제 `convertFileSrc`로 WAV 1개 로드**해 P3-A 완료 시점에 검증한다. 안 맞으면 공식 문서의 변수 정의에 맞춰 패턴 조정.
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | CLEAN (PLAN) | 2 issues found, 2 resolved, 1 critical gap (orphan UX visibility) |
| Design Review | `/plan-design-review` | UI/UX gaps | 0 | - | not run |
| DX Review | `/plan-devex-review` | Developer experience gaps | 0 | - | not run |

- **UNRESOLVED:** 0 (이번 라운드에서 제기된 문서 정합성 이슈는 모두 반영 완료)
- **VERDICT:** ENG REVIEW CLEARED (PLAN) - P3-A+B 구현 착수 가능. 단, orphan UX 가시성은 TODO로 추적 권장.
